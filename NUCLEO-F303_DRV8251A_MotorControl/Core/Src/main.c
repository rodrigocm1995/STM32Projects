...
int main(void)
{
  ...
  motor.channel2 = TIM_CHANNEL_2; // IN2 -> PB5
  motor.max_duty = htim3.Instance->ARR;
  motor.nFaultPort = GPIOC;
  motor.nFaultPin = GPIO_PIN_13;

  motor.hadc = &hadc1;
  motor.adc_buffer = adc_dma_buffer; // Puntero al buffer
  motor.adc_buffer_size = ADC_BUF_SIZE;
  motor.r_ipropi = 1000.0f;
  motor.vref = 3.3f;
