...
volatile uint8_t isConversionCompleted = 0;
...
int main(void)
{
  ...
  while (1)
  {
    // Limpiar la bandera para esperar el siguiente ciclo
    if (isConversionCompleted)
    {
        ...
        if (rawVref > 0)
        {
            ...
            // Formatear la cadena y transmitirla por el puerto serie USART2 (Virtual COM)
            snprintf(msg, sizeof(msg), "Temp: %.2f C\r\n", temperature);
            HAL_UART_Transmit(&huart2, (uint8_t*)msg, strlen(msg), HAL_MAX_DELAY);
        }
        // Retardo de 1 segundo para no inundar la consola serial.
        // (Nota: Durante este segundo, el hardware del ADC y el DMA siguen actualizando
        // el arreglo adc_buffer en segundo plano sin intervención del CPU).
        HAL_Delay(1000);
    }
    /* USER CODE END WHILE */
