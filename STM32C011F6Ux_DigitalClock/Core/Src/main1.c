...
/**
  ******************************************************************************
  * @file           : main1.c
  * @brief          : Main program body (With Power Outage Protection)
  ******************************************************************************
  * @attention
  *
...
volatile _Bool flag20ms = 0; // Bandera de 20 ms levantada por TIM3
...
int main(void)
{
  ...
  while (1)
  {
    __WFI(); // Entra en modo sleep de bajo consumo, despierta con TIM3 (5ms) o RTC Alarm (1s) o EXTI
...
void SystemClock_Config(void)
{
  ...
  */
  __HAL_RCC_LSEDRIVE_CONFIG(RCC_LSEDRIVE_MEDIUMHIGH); // Se habilita la capacidad de cambio de registro de LSE Drive
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA, COLON_Pin|THOU_Pin|HUND_Pin|TENS_Pin
                           |UNIT_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : COLON_Pin THOU_Pin HUND_Pin TENS_Pin
                           UNIT_Pin */
  GPIO_InitStruct.Pin = COLON_Pin|THOU_Pin|HUND_Pin|TENS_Pin
                           |UNIT_Pin;
  ...
  /*Configure GPIO pin : POWER_AUTAGE_Pin */
  GPIO_InitStruct.Pin = POWER_AUTAGE_Pin;
  /* IMPORTANTE: Configuramos para disparar tanto por RISING como por FALLING
     para poder detectar la pérdida de energía y el retorno de la misma. */
  GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING_FALLING;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(POWER_AUTAGE_GPIO_Port, &GPIO_InitStruct);
...
/* --- Callback EXTI para Pines de Entrada --- */
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  if (GPIO_Pin == POWER_AUTAGE_Pin)
  {
    /* DETECCIÓN DE CORTE DE ENERGÍA Y BAJO CONSUMO (MODO STOP)
       Asumimos que:
       - LOW  (0) indica "Corte de energía" (el sistema corre con batería).
       - HIGH (1) indica "Energía presente" (el sistema corre con fuente externa).
     */
    if (HAL_GPIO_ReadPin(POWER_AUTAGE_GPIO_Port, POWER_AUTAGE_Pin) == GPIO_PIN_RESET)
    {
      // Apagar la pantalla inmediatamente para no drenar la pila
      Display_Off();

      // Detener temporizadores de multiplexado para ahorrar energía
      HAL_TIM_Base_Stop_IT(&htim3);

      // Suspender el tick del sistema (SysTick) para evitar despertares parasitarios
      HAL_SuspendTick();

      // Entrar en modo STOP de bajo consumo. La CPU se congela aquí.
      HAL_PWR_EnterSTOPMode(PWR_LOWPOWERREGULATOR_ON, PWR_STOPENTRY_WFI);

      /* =========================================================================
         -- EL MICROCONTROLADOR SE DESPIERTA AQUÍ CUANDO LA ENERGÍA RETORNA --
         (Al retornar la energía, el pin PB7 baja a LOW, generando un flanco de bajada
         que activa de nuevo la interrupción EXTI y despierta al micro de STOP).
         ========================================================================= */

      // Al despertar, el micro vuelve al oscilador interno básico (HSI). 
      // Debemos reconfigurar los relojes del sistema a su velocidad original.
      SystemClock_Config();

      // Reanudar SysTick y el temporizador de la pantalla
      HAL_ResumeTick();
      HAL_TIM_Base_Start_IT(&htim3);
    }
  }
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  if (htim->Instance == TIM3)
  {
    ...
    // Divisor de frecuencia para el boton y segmentos (4 * 5ms = 20ms)
